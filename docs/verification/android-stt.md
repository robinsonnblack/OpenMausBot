...
Tap the microphone once to start; tap again to finish and await transcription. Starting, recording and processing have separate visible states. Leaving the chat or tapping during processing cancels. Choose **Erneutes Tippen** (default) or **Sprechpause**, with a positive pause length in milliseconds (default 5000). Initial silence does not stop a recording. Choose **Nur einfügen** (default) or **Direkt senden**; automatic sending occurs once, after a successful nonempty final result, never after a partial, error or cancellation. Failed sends retain the transcript.
...
The pause editor retains raw text and selection while editing (5000 → 000 → 2000). The expanded sheet uses available screen height instead of a fixed 560 dp editor cap. A German 360 × 800 dp fixture asserts completion choices are fully visible without scrolling. Activity choices and their help use localized resources. Existing user-selected pause lengths remain unchanged when defaults change.
