...
internal fun shareUiText(context: Context, text: String): String = when (text) {
    ...
    "1 unsupported item was left out." -> context.getString(R.string.android_share_ignored_one)
        ...
        text.matches(Regex("Attach up to [0-9]+ images at a time\\.")) -> context.getString(R.string.image_attachment_count_error, text.removePrefix("Attach up to ").substringBefore(" images"))
        text.startsWith("Those images are larger than ") && text.endsWith(" MB together.") -> context.getString(R.string.image_attachment_size_error, text.removePrefix("Those images are larger than ").substringBefore(" MB together."))
