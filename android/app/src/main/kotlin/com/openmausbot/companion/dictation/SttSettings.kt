...
class SttStore(context: Context) {
    ...
    fun load(): SttConfig = try { prefs.getString("stt.settings", null)?.let { CompanionJson.decodeFromString<SttConfig>(it).validated().also { cfg -> require(STT_PROVIDERS.any { p -> p.id == cfg.provider }) } } ?: SttConfig() } catch (_: Exception) { error("STT settings could not be read.") }
