  ...
  return (
    <aside className="animate-panel-in relative z-20 flex h-full w-[400px] shrink-0 flex-col border-l border-hairline/40 bg-panel">
      <div className={cn("flex items-center justify-between px-4 py-3", padClass)}>
        <button
          onClick={close}
          aria-label={t("hardcoded.components.RemoteAgentSettingsPanel.3d06d11c")}
          className="flex size-10 items-center justify-center rounded-md text-ink-secondary hover:bg-control hover:text-ink"
        >
          <ChevronLeft size={18} />
        </button>
        <span className="text-[15px] font-semibold text-ink">{t("hardcoded.components.RemoteAgentSettingsPanel.2c98753b")}</span>
        <button
          onClick={close}
          aria-label={t("hardcoded.components.RemoteAgentSettingsPanel.559960ae")}
          className="flex size-10 items-center justify-center rounded-md text-ink-secondary hover:bg-control hover:text-ink"
        >
          <X size={18} />
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5">
        <div className="flex flex-col gap-4 pt-4">
          <div className="rounded-xl bg-card p-4">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" disabled={uploading || saving} onClick={() => avatarInput.current?.click()} className="flex items-center gap-1.5 rounded-lg bg-control px-3 py-2 text-[13px] text-ink hover:bg-raised-hover disabled:opacity-50">
                  {uploading ? <Loader2 size={14} className="animate-spin" /> : <ImagePlus size={14} />} {t("hardcoded.components.RemoteAgentSettingsPanel.9dbf9b91")}
                </button>
                {bot.avatarUrl && <button type="button" disabled={saving} onClick={() => void patch({ avatarUrl: null, avatarCrop: "mascot" })} aria-label={t("hardcoded.components.RemoteAgentSettingsPanel.f655d561")} className="flex size-9 items-center justify-center rounded-lg text-ink-secondary hover:bg-control hover:text-danger"><Trash2 size={14} /></button>}
              </div>
            </div>
            <label className="block text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.RemoteAgentSettingsPanel.7a4ec6eb")}
            ...
            <label className="mt-3 block text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.RemoteAgentSettingsPanel.7a465f8d")}
            ...
            <label className="mt-3 block text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.RemoteAgentSettingsPanel.a6ae0e98")}
          ...
          <div className="flex items-center justify-between gap-4 rounded-xl bg-card p-4">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div>
                <div className="text-[15px] font-medium text-ink">{t("hardcoded.components.RemoteAgentSettingsPanel.3d3ce84f")}</div>
                <div className="mt-0.5 text-[12px] leading-relaxed text-ink-secondary">
                  {t("hardcoded.components.RemoteAgentSettingsPanel.1bd89f8f")}
                </div>
              </div>
            </div>
            <Switch
              ...
              disabled={saving}
              aria-label={t("hardcoded.components.RemoteAgentSettingsPanel.a1fd590a")}
