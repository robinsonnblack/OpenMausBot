...
export function MessageDeletion({ threadId, messages }: { threadId: string; messages: Message[] }) {
  ...
  return <>
    <button type="button" onClick={() => void show()} title={t("hardcoded.components.MessageDeletion.c1daca7b")} aria-label={t("hardcoded.components.MessageDeletion.c1daca7b")}
    ...
    {open && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-5" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !working) setOpen(false); }}>
      <section role="dialog" aria-modal="true" aria-label={t("hardcoded.components.MessageDeletion.c1daca7b")} className="flex max-h-[80vh] w-full max-w-xl flex-col rounded-2xl border border-hairline bg-panel p-5 text-ink shadow-2xl">
        <div className="flex items-center justify-between"><h2 className="text-lg font-semibold">{t("hardcoded.components.MessageDeletion.c1daca7b")}</h2>
          <button type="button" aria-label={t("hardcoded.components.MessageDeletion.fa5755b5")} onClick={() => setOpen(false)} disabled={working} className="rounded p-1 hover:bg-raised"><X size={18} /></button></div>
        <p className="mt-2 text-sm text-ink-secondary">{t("hardcoded.components.MessageDeletion.b6b0d471")}</p>
        <div className="mt-3 flex gap-2 text-sm">
          <button type="button" onClick={() => setSelected(new Set(selection?.allIds ?? []))} disabled={!selection || working} className="rounded px-2 py-1 hover:bg-raised">{t("hardcoded.components.MessageDeletion.73de5fa7")}</button>
          <button type="button" onClick={() => setSelected(new Set())} disabled={working || selected.size === 0} className="rounded px-2 py-1 hover:bg-raised">{t("hardcoded.components.MessageDeletion.de17c1c1")}</button>
          <span className="ml-auto py-1 text-ink-secondary">{selected.size} {t("hardcoded.components.MessageDeletion.06ea457c")}</span>
        </div>
        <div className="mt-2 min-h-20 flex-1 overflow-y-auto rounded-lg border border-hairline p-2">
          {!selection && !error && <p className="p-2 text-sm text-ink-secondary">{t("hardcoded.components.MessageDeletion.f24f5dd4")}</p>}
          ...
          {selection?.allIds.length === 0 && <p className="p-2 text-sm text-ink-secondary">{t("hardcoded.components.MessageDeletion.359b9fa9")}</p>}
        </div>
        {error && <p role="alert" className="mt-2 text-sm text-red-400">{error}</p>}
        <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setOpen(false)} disabled={working} className="rounded-lg px-3 py-2 hover:bg-raised">{t("hardcoded.components.MessageDeletion.f17a228c")}</button>
    ...
    <ConfirmDialog open={confirm} title={`Delete ${selected.size} messages?`} body="This permanently deletes the selected messages and cannot be undone." confirmLabel={t("hardcoded.components.MessageDeletion.c1daca7b")} tone="danger" onCancel={() => setConfirm(false)} onConfirm={() => void remove()} />
  </>;
}
