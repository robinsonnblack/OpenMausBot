  ...
  return <div ref={panel}><Card title={`Computer access · ${workspace.name}`} subtitle={workspace.origin}>
    <div className="flex flex-col gap-4 text-[13px]">
      <div className="flex items-center justify-between gap-2">
        ...
        <button type="button" aria-label={t("hardcoded.components.ComputerSharingSettings.20b153aa")} onClick={onClose} className="rounded p-1 text-ink-secondary hover:bg-control"><X size={16} /></button>
      </div>
      <p className="text-ink-secondary">{t("hardcoded.components.ComputerSharingSettings.456c1717")}</p>
      <fieldset disabled={busy || !state} className="flex flex-col gap-3 disabled:opacity-50">
        <legend className="mb-2 font-medium text-ink">{t("hardcoded.components.ComputerSharingSettings.00ba7995")}</legend>
        {folders.length === 0 && <p className="text-[12px] text-ink-secondary">{t("hardcoded.components.ComputerSharingSettings.ad91dbad")}</p>}
        {folders.map(folder => <div key={folder.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-hairline/40 p-3">
          ...
          <label className="flex items-center gap-2 text-[12px] text-ink-secondary"><input type="checkbox" checked={folder.write} onChange={event => setFolders(current => current.map(entry => entry.id === folder.id ? { ...entry, write: event.target.checked } : entry))} />{t("hardcoded.components.ComputerSharingSettings.5a19fc68")}</label>
        ...
        })} className="flex w-fit items-center gap-2 rounded-lg border border-hairline/40 px-3 py-2 text-ink hover:bg-control"><FolderPlus size={14} />{t("hardcoded.components.ComputerSharingSettings.2dec4966")}</button>
        <p className="text-[12px] text-ink-secondary">{t("hardcoded.components.ComputerSharingSettings.240830aa")}</p>
      </fieldset>
      <fieldset disabled={busy || !state} className="flex flex-col gap-3 border-t border-hairline/40 pt-3 disabled:opacity-50">
        <legend className="font-medium text-ink">{t("hardcoded.components.ComputerSharingSettings.ddd4a98f")}</legend>
        <label className="flex items-start gap-2"><input className="mt-1" type="checkbox" checked={terminal} onChange={event => setTerminal(event.target.checked)} /><span className="text-ink">{t("hardcoded.components.ComputerSharingSettings.c463a2aa")}<span className="mt-1 block text-[12px] text-ink-secondary">{t("hardcoded.components.ComputerSharingSettings.b529d6c9")}</span></span></label>
        <label className="flex items-start gap-2"><input className="mt-1" type="checkbox" checked={computer} onChange={event => setComputer(event.target.checked)} /><span className="text-ink">{t("hardcoded.components.ComputerSharingSettings.7e3b4ea8")}<span className="mt-1 block text-[12px] text-ink-secondary">{t("hardcoded.components.ComputerSharingSettings.8398e970")}</span></span></label>
      </fieldset>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        })} className="rounded-lg border border-hairline/40 px-3 py-2 text-danger disabled:opacity-50">{t("hardcoded.components.ComputerSharingSettings.9329f4e7")}</button>}
      </div>
      {state?.enabled && <p className="text-[12px] text-ink-secondary">{t("hardcoded.components.ComputerSharingSettings.6110cb17")}</p>}
    </div>
  </Card></div>;
}
