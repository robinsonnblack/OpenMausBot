    ...
    <div className="block">
      <div className="mb-1.5 flex items-center justify-between gap-3">
        <label htmlFor={`bot-soul-${bot.id}`} className="text-[13px] text-ink-secondary">
          {t("hardcoded.components.SoulField.b02c2fa8")}
        </label>
        {canMigrate && (
          <button
            type="button"
          ...
          >
            {t("hardcoded.components.SoulField.865277eb")}
          </button>
        )}
      </div>
      {info?.drift && (
        <div className="mb-2 rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-[12px] text-ink">
          <div className="font-medium">{t("hardcoded.components.SoulField.79482025")}</div>
          <div className="mt-1 text-ink-secondary">
            {t("hardcoded.components.SoulField.abb3aff2")} <span className="break-all">{info.file}</span>
          </div>
          ...
          <div className="mt-2 flex gap-2">
            <button type="button" disabled={resolving} onClick={() => void resolve("apply-file")} className="rounded-lg bg-accent px-3 py-1.5 text-[12px] font-medium text-white hover:brightness-110 disabled:opacity-50">
              {t("hardcoded.components.SoulField.af78755d")}
            </button>
            <button type="button" disabled={resolving} onClick={() => void resolve("discard-file")} className="rounded-lg bg-control px-3 py-1.5 text-[12px] text-ink hover:bg-raised-hover disabled:opacity-50">
              {t("hardcoded.components.SoulField.3cecf90e")}
            </button>
          </div>
        </div>
      )}
      <textarea
        ...
        placeholder={t("hardcoded.components.SoulField.43b8e169")}
      ...
      <div className="mt-1.5 flex items-start justify-between gap-3 text-[11px] text-ink-secondary">
        <span>
          {t("hardcoded.components.SoulField.6354da83")}{info?.file ? <> {t("hardcoded.components.SoulField.93348aa7")} <span className="break-all">{info.file}</span>.</> : null}
        </span>
        <span className={cn("shrink-0 tabular-nums", over && "font-medium text-red-500")}>
          {bytes.toLocaleString()} / {limit.toLocaleString()} {t("hardcoded.components.SoulField.8f600593")}{over ? " — not saved" : ""}
        </span>
      </div>
    </div>
  );
