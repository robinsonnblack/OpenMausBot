  ...
  return (
    <>
      ...
      {Boolean(pending.warnings?.length) && <div className="mt-4 rounded-xl border border-hairline px-4 py-3 text-[12.5px] text-ink-secondary">
        <div className="mb-2 font-medium text-ink">{t("hardcoded.components.TeamLibraryPanel.49c67217")}</div>
      ...
      {(pending.kind === "package" || pending.kind === "backup") && (
        <div className="mt-5 flex flex-wrap gap-2 text-[11.5px] text-ink-secondary">
          {pending.chiefOfStaff && <span className="flex items-center gap-1.5 rounded-full bg-raised px-3 py-1.5"><Crown size={13} />{pending.chiefOfStaff} {t("hardcoded.components.TeamLibraryPanel.3abbf10d")}</span>}
          ...
          {!(pending.library && !pending.playbooks) && <span className="flex items-center gap-1.5 rounded-full bg-raised px-3 py-1.5"><BookOpen size={13} />{pending.playbooks} {t("hardcoded.components.TeamLibraryPanel.0b60ecaa")}</span>}
          ...
          {pending.kind === "package" && pending.version !== 2 && <span className="flex items-center gap-1.5 rounded-full bg-raised px-3 py-1.5"><Plug size={13} />{pending.apps.length} {t("hardcoded.components.TeamLibraryPanel.1844eb52")}</span>}
      ...
      {pending.members.length > 0 && <div className="mt-6 text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.4487179f")}</div>}
        ...
        {pending.members.map((member, index) => (
          <div key={`${member.name}-${index}`} className="flex min-h-[72px] items-center gap-3 border-b border-hairline/35 px-1 py-3">
            ...
            <div className="min-w-0">
              ...
              {importedNames[index] !== member.name && <div className="text-[11.5px] text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.8f386d26")} {member.name}</div>}
            ...
            <div className="flex items-center gap-2">
              {pending && (
                <button
                  ...
                  aria-label={t("hardcoded.components.TeamLibraryPanel.ee664cbc")}
                >
                  <ArrowLeft size={18} />
                </button>
              )}
          ...
          <div className="flex shrink-0 items-center gap-1">
            {!pending && (
              <button
                ...
                title={t("hardcoded.components.TeamLibraryPanel.330b5e45")}
              >
                <Github size={16} />
                <span className="max-sm:hidden">{t("hardcoded.components.TeamLibraryPanel.1e31d240")}</span>
            ...
            <button
              ...
              aria-label={t("hardcoded.components.TeamLibraryPanel.93c139c4")}
            >
              <X size={21} />
            </button>
          </div>
        ...
        {pending ? (
          <>
            ...
            <footer className="flex flex-col gap-3 border-t border-hairline/35 px-6 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-8">
              <div className="text-[12.5px] text-ink-secondary">
                {t("hardcoded.components.TeamLibraryPanel.ef41d9d5")} {currentBotCount > 0 ? `${currentBotCount} existing ${currentBotCount === 1 ? "bot and its" : "bots and their"}` : "existing"} {t("hardcoded.components.TeamLibraryPanel.40e79fbd")}
                {" "}{pending.kind === "backup"
          ...
          <>
            <div className="flex flex-col gap-3 px-6 pb-4 pt-5 sm:flex-row sm:items-center sm:justify-between sm:px-8">
              <div className="flex w-fit rounded-xl bg-raised/70 p-1" role="tablist" aria-label={t("hardcoded.components.TeamLibraryPanel.8612b2c4")}>
                ...
                >
                  {t("hardcoded.components.TeamLibraryPanel.1b5f673b")}
                </button>
                <button
                  role="tab"
                ...
                >
                  {t("hardcoded.components.TeamLibraryPanel.10b95b13")}
                </button>
                <button
                  role="tab"
                ...
                >
                  {t("hardcoded.components.TeamLibraryPanel.243479b0")}
                </button>
                <button
                  role="tab"
              ...
              {tab === "explore" && (
                <label className="flex h-11 w-full items-center gap-2.5 rounded-xl bg-raised/70 px-3.5 sm:w-[320px]">
                  ...
                  <input
                    ...
                    placeholder={t("hardcoded.components.TeamLibraryPanel.bd7ebb14")}
                    aria-label={t("hardcoded.components.TeamLibraryPanel.bd7ebb14")}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-7 pt-5 sm:px-8">
                ...
                <div>
                  ...
                  {catalogLoading && (
                    <div className="flex items-center justify-center gap-2 py-24 text-[13px] text-ink-secondary">
                      <Loader2 size={16} className="animate-spin" /> {t("hardcoded.components.TeamLibraryPanel.9e7d9347")}
                    </div>
                  )}
                  {!catalogLoading && catalogError && (
                    <div className="rounded-xl bg-danger/10 p-4 text-[13px] text-danger">
                      <p>{catalogError}</p>
                      <button onClick={() => void loadCatalog()} className="mt-3 rounded-full bg-raised px-3.5 py-2 text-ink hover:bg-raised-hover">{t("hardcoded.components.TeamLibraryPanel.a21b0b26")}</button>
                    </div>
                  )}
                        ...
                        <article key={entry.slug} className="flex min-h-[104px] items-center gap-3 border-b border-hairline/35 px-1 py-4">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-1 truncate text-[11.5px] text-ink-secondary/80">
                              {entry.members} {t("hardcoded.components.TeamLibraryPanel.be46eadd")} {entry.skills.length} {t("hardcoded.components.TeamLibraryPanel.0b60ecaa")}
                  ...
                  {!catalogLoading && catalog && visibleTeams.length === 0 && (
                    <div className="flex min-h-56 flex-col items-center justify-center text-center">
                      <div className="text-[14px] font-medium text-ink">{t("hardcoded.components.TeamLibraryPanel.8fa19173")}</div>
                      <div className="mt-1 text-[12.5px] text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.133fb102")}</div>
                    </div>
                  )}
                </div>
              )}
              ...
              {tab === "import" && (
                <div>
                  ...
                  />
                  <div className="mb-3 text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.ccc1db95")}</div>
                  <div className="grid gap-5 md:grid-cols-2">
                    ...
                    >
                      ...
                      <span className="mt-3 text-[14px] font-medium text-ink">{t("hardcoded.components.TeamLibraryPanel.ccfc44f4")}</span>
                      <span className="mt-1 text-[12.5px] text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.223d509f")}</span>
                    </button>
                    ...
                    <div className="flex min-h-56 flex-col justify-center rounded-2xl bg-raised/25 px-6">
                      ...
                      <h3 className="mt-3 text-[14px] font-medium text-ink">{t("hardcoded.components.TeamLibraryPanel.54fbeb2e")}</h3>
                      <p className="mt-1 text-[12.5px] leading-relaxed text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.4aa93fef")}</p>
                      <div className="mt-4 flex gap-2">
                        <input
                          value={githubUrl}
                          onChange={(event) => setGithubUrl(event.target.value)}
                          onKeyDown={(event) => event.key === "Enter" && void loadGithubTeam()}
                          placeholder={t("hardcoded.components.TeamLibraryPanel.826cad12")}
                          aria-label={t("hardcoded.components.TeamLibraryPanel.feef12a1")}
                        ...
                        >
                          ...
                          {t("hardcoded.components.TeamLibraryPanel.e193fbe4")}
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              {tab === "scout" && (
                <div>
                  <div className="mb-3 text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.5a5ae531")}</div>
                  <p className="max-w-2xl text-[12.5px] leading-relaxed text-ink-secondary">
                    {t("hardcoded.components.TeamLibraryPanel.192c6a49")}
                  </p>
                  <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                    <input
                      ...
                      aria-label={t("hardcoded.components.TeamLibraryPanel.87592958")}
                      className="min-w-0 flex-1 rounded-xl bg-raised/80 px-3 py-2.5 text-[13px] text-ink placeholder:text-ink-secondary focus:outline-none"
                    />
                    {Boolean(window.ogb?.pickFolder) && (
                      ...
                      >
                        <FolderOpen size={14} />
                        {t("hardcoded.components.TeamLibraryPanel.7a74fdc5")}
                      </button>
                    )}
                    <button
                  ...
                  {scouted && (
                    <div className="mt-6">
                      ...
                      <div className="mt-5 text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.c0351645")}</div>
                      ...
                      {directory && directory.length > 0 && (
                        <>
                          <div className="mt-5 text-[12px] font-medium text-ink-secondary">{t("hardcoded.components.TeamLibraryPanel.0452e16b")}</div>
                              ...
                              <div key={candidate.slug} className="flex items-center gap-3 border-b border-hairline/35 px-1 py-3">
                                <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-3">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    <div className="mt-0.5 truncate text-[12px] text-ink-secondary">
                                      {t("hardcoded.components.TeamLibraryPanel.5d9b8fdd")} {candidate.matched.join(", ")}
                                    </div>
                                  </div>
                                </label>
                                <button
                                  ...
                                  title={t("hardcoded.components.TeamLibraryPanel.859755cc")}
                      ...
                      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
                        <input
                          ...
                          aria-label={t("hardcoded.components.TeamLibraryPanel.6a38f6c0")}
                          className="min-w-0 flex-1 rounded-xl bg-raised/80 px-3 py-2.5 text-[13px] text-ink placeholder:text-ink-secondary focus:outline-none"
                        />
                        <button
                      ...
                      <p className="mt-2 text-[12px] text-ink-secondary">
                        {t("hardcoded.components.TeamLibraryPanel.26709e65")}
                      </p>
                    </div>
                  )}
                  {error && <div role="alert" className="mt-4 rounded-lg bg-danger/10 px-3 py-2 text-[12.5px] text-danger">{error}</div>}
