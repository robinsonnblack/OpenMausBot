...
export function VoiceSettings({
  ...
  return (
    <div className="rounded-xl bg-card p-4">
      <div className="text-[15px] font-medium text-ink">{t("hardcoded.components.VoiceSettings.fb985349")}</div>
      <div className="mt-0.5 text-[13px] text-ink-secondary">
        {localMacClient
          ...
          : workspaceConfigurationLocked
            ...
            : <>{t("hardcoded.components.VoiceSettings.61492a03")}
              {provider === "system"
      ...
      {localMacClient && (
        <div className="mt-4">
          <div className="mb-2 text-[13px] text-ink-secondary">{t("hardcoded.components.VoiceSettings.86f25fb5")}</div>
          <div className="inline-flex rounded-xl bg-inset p-1" role="radiogroup" aria-label={t("hardcoded.components.VoiceSettings.86f25fb5")}>
            {([
      ...
      {!workspaceConfigurationLocked && (
        <div className="mt-4">
          <div className="mb-2 text-[13px] text-ink-secondary">{t("hardcoded.components.VoiceSettings.158e0219")}</div>
          <div className="grid grid-cols-2 gap-1 rounded-xl bg-inset p-1" role="radiogroup" aria-label={t("hardcoded.components.VoiceSettings.158e0219")}>
            {([
      ...
      {!workspaceConfigurationLocked && cloudProvider && (
        ...
        <div className="mb-1.5 flex items-center gap-2 text-[13px] text-ink-secondary">
          ...
          <span>{cloudProvider.name} {t("hardcoded.components.VoiceSettings.8c86d7ad")}</span>
          {configured && <span className="text-[11px] text-success">{t("hardcoded.components.VoiceSettings.cebf85d7")}</span>}
        </div>
        <div className="flex gap-2">
          ...
          >
            {saving ? <Loader2 size={13} className="animate-spin" /> : <><Check size={13} />{t("hardcoded.components.VoiceSettings.e16364dd")}</>}
          </button>
        </div>
        {!configured && (
          <a
          ...
          >
            {t("hardcoded.components.VoiceSettings.da351da3")} {cloudProvider.name}
          </a>
        )}
        </div>
      )}
      ...
      {!workspaceConfigurationLocked && provider === "chatterbox" && (
        ...
        <div className="mb-1.5 flex items-center gap-2 text-[13px] text-ink-secondary">
          <span className={cn("size-1.5 rounded-full", configured ? "bg-success" : "bg-raised-hover")} />
          <span>{t("hardcoded.components.VoiceSettings.0fb36b34")}</span>
          {configured && <span className="text-[11px] text-success">{t("hardcoded.components.VoiceSettings.98e66115")}</span>}
        </div>
        <div className="flex gap-2">
          <input
            type="url"
            ...
            placeholder={t("hardcoded.components.VoiceSettings.03e1dce6")}
            aria-label={t("hardcoded.components.VoiceSettings.cb17712b")}
            autoComplete="off"
            spellCheck={false}
            className="w-full rounded-lg border border-hairline/40 bg-inset px-3 py-2 text-[13px] text-ink placeholder:text-ink-secondary focus:border-hairline focus:outline-none"
          />
          <button
            ...
            className="flex w-[72px] shrink-0 items-center justify-center gap-1.5 rounded-lg bg-control py-2 text-[13px] text-ink hover:bg-raised-hover disabled:cursor-not-allowed disabled:opacity-50"
          >
            {savingServer ? <Loader2 size={13} className="animate-spin" /> : <><Check size={13} />{t("hardcoded.components.VoiceSettings.e16364dd")}</>}
          </button>
        </div>
        <input
          type="text"
          ...
          onKeyDown={(e) => e.key === "Enter" && void saveServer()}
          placeholder={t("hardcoded.components.VoiceSettings.6ae0df10")}
          aria-label={t("hardcoded.components.VoiceSettings.a1efe789")}
          autoComplete="off"
          spellCheck={false}
        ...
        <div className="mt-1.5 text-[11.5px] leading-relaxed text-ink-secondary">
          {t("hardcoded.components.VoiceSettings.1939897d")}{" "}
          <a
          ...
          >
            {t("hardcoded.components.VoiceSettings.dd35578c")}
          </a>
        </div>
        </div>
      )}
      ...
      {configured && (
        <div className="mt-4">
          <div className="mb-1.5 text-[13px] text-ink-secondary">{t("hardcoded.components.VoiceSettings.fb985349")}</div>
          <div className="flex gap-2">
            <select
              value={selectedVoice}
            ...
            >
              ...
              {selectedVoice && !voices.some((voice) => voice.id === selectedVoice) && (
                <option value={selectedVoice}>{t("hardcoded.components.VoiceSettings.e5c4608d")}</option>
              )}
              {voices.map((v) => (
            ...
            <button
              ...
              aria-label={t("hardcoded.components.VoiceSettings.4b996e62")}
              className="flex w-[72px] shrink-0 items-center justify-center gap-1.5 rounded-lg bg-control py-2 text-[13px] text-ink hover:bg-raised-hover disabled:cursor-not-allowed disabled:opacity-50"
            >
              <Volume2 size={14} /> {t("hardcoded.components.VoiceSettings.b46a662b")}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 flex items-center justify-between gap-4 border-t border-hairline/40 pt-4">
        <div>
          <div className="text-[13px] font-medium text-ink">{t("hardcoded.components.VoiceSettings.7867bf79")}</div>
          <div className="mt-0.5 text-[11.5px] leading-relaxed text-ink-secondary">
            {t("hardcoded.components.VoiceSettings.e0cc227b")}
          </div>
        </div>
        <Switch
